...
{
    public class BELargeOpenShelfDoorCabinet : BlockEntityDisplay
    {
        public List<BlockPos> DummyPositions { get; set; } = new List<BlockPos>();
        InventoryDrawer inv;
        public override InventoryDrawer Inventory => inv;
        public override string InventoryClassName => "largeopenshelfdoorcabinet";

        public override string AttributeTransformCode => "oncrateclosedTransform";
        public override int DisplayedItems => 48;
        Block block;
        GuiDialogDrawers storageDlg;

        static int slotCount = 48;
        IPlayer byPlayer;

        private BlockEntityAnimationUtil animUtil => GetBehavior<BEBehaviorAnimatable>()?.animUtil;
        private bool _isOpen = false;

        public BELargeOpenShelfDoorCabinet()
        {
            inv = new InventoryDrawer("largeopenshelfdoorcabinet-0", slotCount, null);
        }
        ...
        public override void Initialize(ICoreAPI api)
        {
            ...
            if (api.Side == EnumAppSide.Client)
            {
                ...
                AssetLocation shapeLoc = new AssetLocation("moreinventorys:shapes/largeopenshelfdoorcabinet.json");
                Shape shape = Shape.TryGet(capi, shapeLoc);
                if (shape == null) return;

                animUtil?.InitializeAnimator("largeopenshelfdoorcabinet", shape, null, new Vec3f(0, block.Shape.rotateY, 0));
            }
        }
        ...
        public override void OnReceivedServerPacket(int packetid, byte[] data)
        {
            ...
            if (packetid == 1000)
            {
                ...
                {
                    Open(); 
                    storageDlg = new GuiDialogDrawers(Lang.Get("moreinventorys:block-largeopenshelfdoorcabinet-north"), (InventoryGeneric)Inventory, Pos, Api as ICoreClientAPI, DrawerType.LargeOpenShelfDoorCabinet);
        ...
        protected override float[][] genTransformationMatrices()
        {
            ...
            for (int index = 0; index < slotCount; index++)
            {
                ...
                else if (index <= 11)
                {
                    ...
                    if (index == 11) x = baseX2;
                }
                else if (index <= 17) //Открытая полка слоты 
                {
                    x += 0.25f;
                    z = 0.05f;
                    y = 0.68f;
                    ...
                    tfMatrices[index] = new Matrixf()
                       ...
                       .Scale(scale, scale, scale)
                       .Values;
                    if (index == 17) x = baseX2;
                }
                else if (index <= 23) //Открытая полка слоты 
                {
                    x += 0.25f;
                    z = 0.4f;
                    y = 0.68f;
                    ...
                    tfMatrices[index] = new Matrixf()
                       ...
                       .Scale(scale, scale, scale)
                       .Values;
                    if (index == 23) x = baseX2;
                }
                else if (index <= 29) //закрытая полка 1, 
                {
                    x += 0.25f;
                    z = 0.05f;
                    y = 0.38f;
                    ...
                    tfMatrices[index] = new Matrixf()
                       ...
                       .Scale(scale, scale, scale)
                       .Values;
                    if (index == 29) x = baseX2;
                }
                else if (index <= 35) //закрытая полка 1
                {
                    x += 0.25f;
                    z = 0.4f;
                    y = 0.38f;
                    ...
                    tfMatrices[index] = new Matrixf()
                       ...
                       .Scale(scale, scale, scale)
                       .Values;
                    if (index == 35) x = baseX2;
                }
                else if (index <= 41) //закрытая полка 2
                {
                    x += 0.25f;
                    z = 0.05f;
                    y = 0.12f;
                    ...
                    tfMatrices[index] = new Matrixf()
                       ...
                       .Scale(scale, scale, scale)
                       .Values;
                    if (index == 41) x = baseX2;
                }
                else if (index <= 47) //закрытая полка 2
                {
                    x += 0.25f;
                    z = 0.4f;
                    y = 0.12f;
